...
def test_policy_rejects_surrounding_whitespace() -> None:
    result = validate_password_policy(" Sup3rSecretPhrase ")

    assert any("whitespace" in issue for issue in result.issues)


def test_policy_rejects_overlong_password() -> None:
    result = validate_password_policy("Aa1" + "x" * 400)

    assert any("longer than" in issue for issue in result.issues)


def test_policy_rejects_common_password() -> None:
    result = validate_password_policy("Password1")
...
def test_hash_password_produces_verifiable_hash() -> None:
    ...
    assert password_hash.iterations == FAST_ITERATIONS
...
def test_password_hash_dict_never_exposes_full_hash() -> None:
    ...
    assert len(payload["hash_preview"]) == 11
...
def test_session_token_requires_entropy() -> None:
...
def test_mask_secret_rejects_negative_visible_characters() -> None:
